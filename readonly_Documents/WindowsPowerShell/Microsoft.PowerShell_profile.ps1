...
function Get-NetHosts {
    ...
    $uniqueHosts = New-Object System.Collections.Generic.HashSet[string] -ArgumentList ([System.StringComparer]::OrdinalIgnoreCase)
...
$canConnectToGitHub = try {
    $ping = New-Object System.Net.NetworkInformation.Ping
    $reply = $ping.Send("github.com", 1000) # 1000ms = 1 second
    $reply.Status -eq 'Success'
} catch {
    $false
}
...
# Load scoop and zoxide hooks if available
if (Test-Path (Join-Path $env:USERPROFILE '\scoop\apps\scoop\current\shims\scoop-search.ps1')) {
    Invoke-Expression (&scoop-search --hook)
}
if (Test-Path (Join-Path $env:USERPROFILE '\scoop\apps\zoxide\current\bin\zoxide.exe')) {
